...
function RIA_RuneButton_Update (self, rune, dontFlash)
	...
	if ( (not dontFlash) and (runeType) and (runeType ~= self.rune.runeType)) then
		...
		RIA_RuneButton_ShineFadeIn(self.shine)
	end
...
function RIA_RuneButton_OnEnter(self)
...
function RIA_RuneButton_OnLeave(self)
	GameTooltip:Hide();
end

function RIA_RuneFrame_OnLoad (self)
...
function RIA_RuneFrame_OnEvent (self, event, ...)
	if ( event == "PLAYER_ENTERING_WORLD" ) then
		for i=1,MAX_RUNES do
			local runeButton = _G["RIA_RuneButtonIndividual"..i];
			if runeButton then
				RIA_RuneButton_Update(runeButton, i, true);
			end
		end
	elseif ( event == "RUNE_POWER_UPDATE" ) then
		local runeIndex, isEnergize = ...;
		if runeIndex and runeIndex >= 1 and runeIndex <= MAX_RUNES  then 
			local runeButton = _G["RIA_RuneButtonIndividual"..runeIndex];
			...
			if not runeReady  then
				...
				RIA_RuneButton_ShineFadeIn(runeButton.shine)
			end
	...
	elseif ( event == "RUNE_TYPE_UPDATE" ) then
		local runeIndex = ...;
		if ( runeIndex and runeIndex >= 1 and runeIndex <= MAX_RUNES ) then
			RIA_RuneButton_Update(_G["RIA_RuneButtonIndividual"..runeIndex], runeIndex);
		end
	end
end

function RIA_RuneButton_ShineFadeIn(self)
	...
	mode = "IN",
	timeToFade = 0.5,
	finishedFunc = RIA_RuneButton_ShineFadeOut,
	finishedArg1 = self,
	}
	self.shining=true;
...
function RIA_RuneButton_ShineFadeOut(self)
	self.shining=false;
