...
function RIA:OnLoad()
    ...
    --imagetimer = 4
    --self.eventFrame:SetScript("OnUpdate", RuneItAll_ImagesOnUpdate)
end
...
--[[function RuneItAll_OnUpdate(self, elapsed)
	local self.cooldown = _G["RuneButtonIndividual"..self:GetID().."Cooldown"]
...
--[[local timing = 0
local function RuneItAll_YATAH(self, elapsed)
	timing = timing - elapsed
	
	if timing < 0 then
		for i = 1, 6 do
			self.runes[i]:SetAlpha(RIADB.alphain)
		end
...
function RIA:setTexture(newValue)
    ...
    elseif newValue == "7" then -- RUNICA RUNES
        ...
        for i = 1, 6 do
            ...
            self.overlay[i]:SetTexture(path.."runica\\self.border.tga")
...
function RuneItAll_VehicleOnUpdate(self, elapsed)
    ...
    if timer < 0 then
		for i = 1, 6 do
			self.runes[i]:Show()
			self.runes[i]:SetAlpha(RIADB.alphaout)
			self.runes[i]:SetScale(RIADB.scale)
		end
...
--[[local imagetimer = 0
local function RuneItAll_ImagesOnUpdate(self, elapsed)
...
function RIA:setLocked(newValue)
    ...
    if newValue == "1" then
		if RIADB.moving == "0" then
			...
			RIA.fonts[1]:SetPoint("CENTER", self.runes[1])
			RIA.fonts[1]:Show()
...
--[[ Credit to Yury's RuneDisplay for self.cooldown code ]]--
...
for i = 1, 6 do
	...
	RIA.cdText[i]:SetPoint("CENTER", self.runes[i])
	RIA.cdText[i]:SetAlpha(1)
	RIA.cdText[i]:Show()
end

--RuneItAll_CDFontSize(RIADB.cdfs)
-- Set font size of self.cooldown count text
...
-- OnUpdate to count down self.cooldown counts
...
-- Set and color the self.cooldown texts
...
-- Calculate the current self.cooldown
